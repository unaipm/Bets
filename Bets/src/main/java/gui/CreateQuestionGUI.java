...
public class CreateQuestionGUI extends JFrame {
	...
	public CreateQuestionGUI(Vector<domain.Event> v, JFrame padre) {
		try {
			jbInit(v);
	...
	private void jbInit(Vector<domain.Event> v) throws Exception {
		...
		chckbxNewCheckBox = new JCheckBox(ResourceBundle.getBundle("Etiquetas").getString("SelectIfTeam"));
		chckbxNewCheckBox.setBounds(185, 240, 344, 23);
		...
		try {
			...
			if (inputQuery.length() > 0) {
				...
				if (inputPrice <= 0)
					...
					facade.createQuestion(event, inputQuery, inputPrice,chckbxNewCheckBox.isSelected());
